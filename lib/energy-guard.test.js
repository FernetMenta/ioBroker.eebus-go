...
describe('BaseEnergyGuard (via EebusEnergyGuard)', () => {
    ...
    describe('calculateLimit()', () => {
        it('should return effectivePct * totalLimit / 100 when greater than failsafeLimit', () => {
        ...
        it('should return totalLimit when effectivePct is 100 and failsafeLimit is small', async () => {
    ...
    describe('createObjects()', () => {
        it('should create channel and all base state objects', async () => {
            ...
            expect(adapter.extendObjectAsync.callCount).to.equal(10);
            ...
            expect(paths).to.include('EnergyGuards.Guard_TestGuard.nominalMax');
            expect(paths).to.include('EnergyGuards.Guard_TestGuard.failsafeDuration');
        });
    ...
    describe('applyLimit()', () => {
        it('should store the calculated limit and update currentLimit state', async () => {
    ...
    describe('deactivateLimit()', () => {
        it('should set currentLimit=0', async () => {
            await guard.applyLimit(32000, 50);
...
describe('EebusEnergyGuard', () => {
    ...
    describe('assignUseCaseClient()', () => {
        it('should not throw when assigning a client', () => {
            const fakeClient = { StartHeartbeat: (_req, _opts, cb) => cb(null, {}) };
...
describe('ManualEnergyGuard', () => {
    ...
    describe('createObjects()', () => {
        ...
        it('should set connected as read-only with role=indicator.connected', async () => {
            await guard.createObjects();

            const connCall = adapter.extendObjectAsync.args.find(a => a[0].endsWith('.connected'));
            expect(connCall[1].common.write).to.equal(false);
