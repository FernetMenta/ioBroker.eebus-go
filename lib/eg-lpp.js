...
class EgLpp {
     ...
     */
    constructor(adapter, config, controlClient, csLppClient, translate, { onGrpcError } = {}) {
    ...
    async #initEnergyGuards() {
        ...
        for (const entry of guardConfigs) {
            if (entry.type === 'eebus') {
                ...
                if (!endpoint) {
                    throw new Error(`AddUseCase returned empty endpoint for LPP guard "${entry.name}"`);
                }
                ...
                try {
                    ...
                    if (eventName === 'UseCaseSupportUpdate') {
                        if (!guard.egLpcClient) {
                            ...
                            await guard.assignUseCaseClient(egLppClient, remoteSki, remoteEntityAddr);
                            log.info(
                        ...
                        if (egLppClient) {
                            ...
                            await this.#adapter.setStateAsync(`${guard.basePath}.failsafeDuration`, durationS, true);
