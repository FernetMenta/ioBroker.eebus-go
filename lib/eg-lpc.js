...
class EgLpc {
     ...
     * @param {string} csLpcInstanceId - CS-LPC instance ID for routing
     ...
     */
    constructor(adapter, config, controlClient, csLpcClient, csLpcInstanceId, translate, { onGrpcError } = {}) {
    ...
    async #handleGuardStateChange(localId, state) {
        ...
        if (stateName === 'percentage') {
            ...
            if (guard instanceof ManualEnergyGuard) {
                log.debug(`LPC manual energy guard "${guardName}" heartbeat write received`);
            ...
            const nominalMax = this.#config.lpcMinimumAssuredPower || 4200;

            if (newSum > nominalMax) {
                log.warn(
                    `LPC rejecting failsafe limit ${newValue}W for guard "${guardName}" — sum would be ${newSum}W, exceeding minimum assured power ${nominalMax}W`,
                );
    ...
    async #initEnergyGuards() {
        ...
        for (const entry of guardConfigs) {
            if (entry.type === 'eebus') {
                ...
                if (!instanceId) {
                    throw new Error(`AddUseCase returned empty instance_id for LPC guard "${entry.name}"`);
                }
                this.#egLpcEndpoints.set(entry.ski, { instanceId, entityAddr });
                log.info(
                    `EG-LPC UseCase added for guard "${entry.name}" at entity [${entityAddr}], instance_id: ${instanceId}`,
                );
    ...
    #subscribeEgLpcEvents() {
        ...
        for (const [ski, { instanceId, entityAddr }] of this.#egLpcEndpoints) {
                ...
                try {
                    ...
                    if (eventName === 'UseCaseSupportUpdate') {
                        if (!guard.egLpcClient) {
                            ...
                            await guard.assignUseCaseClient(egLpcClient, instanceId, remoteSki, remoteEntityAddr);
                            log.info(
            ...
            stream.on('error', err => {
                if (this.#stopped) {
                    log.debug(`EG-LPC event stream closed for SKI=${ski}: ${err.message}`);
                    return;
                }
            ...
            stream.on('end', () => {
                if (this.#stopped) {
                    log.debug(`EG-LPC event stream ended for SKI=${ski} (shutdown)`);
                    return;
                }
    ...
    async #distributeLimit(isActive) {
        ...
        const effectiveLimits = distributeLimit(controlboxLimit, guardData);
        ...
        for (let i = 0; i < this.#guards.length; i++) {
            ...
            await this.#guards[i].applyEffectiveLimit(effectiveLimit, remainingDurationNs);
            log.info(
                `LPC guard "${this.#guards[i].name}": pct=${guardData[i].pct}%, effectiveLimit=${effectiveLimit.toFixed(1)}W${remainingDurationNs > 0 ? `, remainingDuration=${Math.round(remainingDurationNs / 1_000_000_000)}s` : ''}`,
            );
        }
    }
    ...
    async #updateReportedFailsafe() {
        ...
        if (failsafeSum > nominalMax) {
            this.#adapter.log.warn(
                `LPC sum of guard failsafe limits (${failsafeSum}W) exceeds minimum assured power (${nominalMax}W) — devices may violate the grid contract in failsafe`,
            );
        }
...
module.exports = { EgLpc, distributeLimit };
