...
const { addEntity, registerRemoteSki } = require('./grpc-service');
 ...
 *
 * CS-LPP registration, event subscription, energy guard management, and limit distribution
 * are added in later tasks (2.3, 2.4, 2.6, 2.9).
 */
class LppUseCase {
    #adapter;
    #config;
                ...
                try {
                    if (limitW > 0) {
                        ...
                        await callUnary(egLppClient, 'WriteProductionLimit', {
                            ...
                            remote_entity_address: { entity_address: [] },
                            limit: {
                                is_active: true,
                                value: limitW,
    ...
    async #initEnergyGuards() {
        ...
        for (const entry of guardConfigs) {
            if (entry.type === 'eebus') {
                ...
                eebusIndex++;
                try {
                    // Use entity addresses offset from CS at [1]:
                    // LPP guards use [1, 100+index] to avoid collision with LPC guards at [1, N]
                    const entityAddr = [1, 100 + eebusIndex];
                    ...
                    if (endpoint) {
                        ...
                        log.info(
                            `EG-LPP UseCase added for guard "${entry.name}" at entity [1,${100 + eebusIndex}], endpoint: ${endpoint}`,
                        );
                    } else {
            ...
            stream.on('data', async evt => {
                ...
                if (eventName === 'UseCaseSupportUpdate') {
                    ...
                    try {
                        ...
                        log.debug(`LPP heartbeat received for guard "${guard.name}" (SKI=${ski})`);
                    } catch (err) {
 ...
 *
 * @param {number} pendingLimit - The pending production limit value in watts
