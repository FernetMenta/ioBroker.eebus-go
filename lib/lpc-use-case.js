...
const { addCsLpcUseCase, subscribeCsLpcEvents, registerRemoteSki } = require('./grpc-service');
...
class LpcUseCase {
    ...
    async handleGuardStateChange(localId, state) {
        ...
        if (stateName === 'percentage') {
            ...
            // Reject if new sum would exceed contractual nominal max
    ...
    async #initEnergyGuards() {
        ...
        for (const entry of guardConfigs) {
            if (entry.type === 'eebus') {
                ...
                try {
                    ...
                    if (!entityAddr) {
                        log.error(`No EG entity found for LPC guard "${entry.name}" SKI=${entry.ski} — skipping`);
                        continue;
                    }
        ...
        // Report sum of guard failsafe limits to the controlbox
        await this.#updateReportedFailsafe();
    }
    ...
    async #distributeLimit(isActive) {
        ...
        if (!isActive) {
            for (const guard of this.#guards) {
                try {
                    ...
                    log.warn(`Failed to deactivate LPC limit on guard "${guard.name}": ${err.message}`);
                }
            }
        ...
        // Use the stored controlbox limit value (set by #handleDataUpdateLimit)
        ...
        if (!allConnected) {
            log.info('LPC not all guards connected — applying failsafe fallback limits');
        }
        ...
        // Use pure distribution function (handles scaling, redistribution, failsafe floors)
        ...
        for (let i = 0; i < this.#guards.length; i++) {
            ...
            try {
                ...
                log.warn(`Failed to apply limit to LPC guard "${this.#guards[i].name}": ${err.message}`);
            }
        }
    }

    /**
     * Update the failsafe limit reported to the controlbox.
     * The reported value is the sum of all configured guard failsafe limits,
     * capped at the contractual consumption nominal max.
     * Called after guard initialization and whenever a guard's failsafe changes.
     */
    async #updateReportedFailsafe() {
        ...
        try {
            ...
            this.#adapter.log.warn(`Failed to update LPC failsafe limit on controlbox: ${err.message}`);
        }
    }
