    ...
    async #distributeLimit(isActive) {
        ...
        if (!allConnected) {
            ...
            for (const guard of this.#guards) {
                try {
                    ...
                    log.warn(`Failed to apply fallback limit to LPC guard "${guard.name}": ${err.message}`);
                }
            }
            return;
        }
        ...
        for (let i = 0; i < this.#guards.length; i++) {
            ...
            try {
                ...
                const effectiveLimit = this.#guards[i].calculateLimit(controlboxLimit, effectivePct);
                log.info(
                    `LPC guard "${this.#guards[i].name}": pct=${percentages[i]}%, effectivePct=${effectivePct.toFixed(2)}%, effectiveLimit=${effectiveLimit}W`,
                );
            } catch (err) {
