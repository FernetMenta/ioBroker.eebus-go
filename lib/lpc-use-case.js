...
const { addCsLpcUseCase } = require('./grpc-service');
...
class LpcUseCase {
     ...
     * @param {(isActive: boolean, limitValue: number, durationNs: number) => Promise<void>} [callbacks.onLimitUpdate] - Called when limit updates
    ...
    /**
     * Expose the CS-LPC instance ID so EgLpc can include it in RPC calls.
     *
     * @returns {string|null} The CS-LPC instance ID, or null if not yet registered
     */
    get csInstanceId() {
        return this.#csLpcInstanceId;
    }
    ...
    async start() {
        ...
        const csLpcInstanceId = await addCsLpcUseCase(this.#controlClient, [1]);

        // Handle empty instance_id: treat as failed registration
        if (!csLpcInstanceId) {
            throw new Error('CS-LPC use case registration returned empty instance_id');
        }

        log.info(`CS-LPC UseCase added with instance_id: ${csLpcInstanceId}`);

        // Create CS-LPC gRPC client at the same endpoint as the control client
        ...
        this.#csLpcInstanceId = csLpcInstanceId;

        // Configure initial parameters
        const nominalMax = this.#config.contractualConsumptionNominalMax || 32000;
        await callUnary(this.#csLpcClient, 'SetConsumptionNominalMax', {
            instance_id: csLpcInstanceId,
            value: nominalMax,
        });
        ...
        await callUnary(this.#csLpcClient, 'StartHeartbeat', { instance_id: csLpcInstanceId });
    ...
    #subscribeCsLpcEvents() {
        ...
        this.#lpcStream = this.#controlClient.SubscribeUseCaseEvents({
            instance_id: this.#csLpcInstanceId,
        });

        this.#lpcStream.on('data', async evt => {
            try {
                ...
                log.error(`CS-LPC event handler error: ${err.message}\n${err.stack}`);
            }
        });

        this.#lpcStream.on('error', err => {
            log.warn(`CS-LPC event stream error: ${err.message}`);
            // Stream termination triggers heartbeatTimeout per EEBus UC TS §2.7
            if (this.#fsm.can('heartbeatTimeout')) {
                log.warn('CS-LPC stream terminated — triggering heartbeatTimeout');
                this.#fsm.heartbeatTimeout();
            }
        });

        this.#lpcStream.on('end', () => {
            log.warn('CS-LPC event stream ended');
            // Stream termination triggers heartbeatTimeout per EEBus UC TS §2.7
            if (this.#fsm.can('heartbeatTimeout')) {
                log.warn('CS-LPC stream ended — triggering heartbeatTimeout');
                this.#fsm.heartbeatTimeout();
            }
        });
    }
    ...
    async #handleDataUpdateLimit() {
        const log = this.#adapter.log;
        const res = await callUnary(this.#csLpcClient, 'ConsumptionLimit', { instance_id: this.#csLpcInstanceId });
        ...
        // Notify the limit update callback (include duration so guards can forward it)
        await this.#onLimitUpdate(!!limit.is_active, limitValue, durationNs);
    }
    ...
    async #handleDataUpdateHeartbeat() {
        ...
        log.debug(`LPC heartbeat update: withinDuration=${res.is_within_duration}`);
    ...
    async #updateFailsafeLimitState() {
        try {
            ...
            // Distribute failsafe limit to energy guards (no duration — indefinite until state change)
            await this.#onLimitUpdate(true, failsafeLimit, 0);
        } catch (err) {
            if (isTransportError(err)) {
                this.#adapter.log.error(
                    `LPC failsafe limit read gRPC transport error: ${err.message} — triggering reconnect`,
                );
                this.#onGrpcError();
            } else {
    ...
    #startHeartbeatCheckTimer() {
        ...
        this.#heartbeatCheckTimer = this.#adapter.setInterval(async () => {
            try {
                ...
                if (isTransportError(err)) {
                    this.#adapter.log.error(
                        `LPC heartbeat check gRPC transport error: ${err.message} — triggering reconnect`,
                    );
                    this.#onGrpcError();
                } else {
...
module.exports = { LpcUseCase, LPC_STATE, LPC_TRANSITIONS, shouldApproveLimit };
