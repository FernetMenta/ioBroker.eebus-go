...
describe('Feature: add-lpp-use-case, Property 2: Production Limit Approval Logic', () => {
     ...
     * and any incoming pending production limit value, the adapter SHALL approve the limit
     * if and only if the limit value is greater than or equal to the sum of all guard
     * failsafe limits; otherwise it SHALL deny the limit.
     */
                ...
                (guardFailsafeLimits, pendingLimit) => {
                    ...
                    if (pendingLimit >= failsafeSum) {
                        expect(result).to.equal(
                            true,
                            `Expected approval: pendingLimit=${pendingLimit} >= failsafeSum=${failsafeSum}`,
                        );
                    } else {
                        expect(result).to.equal(
                            false,
                            `Expected denial: pendingLimit=${pendingLimit} < failsafeSum=${failsafeSum}`,
                        );
                    }
                },
            ),
    ...
    it('approves when no guards are configured (empty array, sum is 0)', function () {
        ...
        fc.assert(
            fc.property(fc.float({ min: 0, max: 50000, noNaN: true }), pendingLimit => {
                // With no guards, failsafe sum is 0, any non-negative limit should be approved
                const result = shouldApproveLimit(pendingLimit, []);
                expect(result).to.equal(true, `Expected approval with no guards: pendingLimit=${pendingLimit} >= 0`);
            }),
            { numRuns: 100 },
        );
    });
...
describe('Feature: add-lpp-use-case, Property 3: Production Limit Distribution', () => {
    ...
    it('sum of connected guard effectiveLimits equals sum of per-guard max(proportionalShare, failsafeLimit)', function () {
        this.timeout(30000);
        ...
        fc.assert(
            fc.property(totalLimitArb, guardsArb, (totalLimit, guards) => {
                ...
                // Replicate the algorithm's effective percentage calculation to derive upper bound
                ...
                const connectedPctSum = connectedPcts.reduce((acc, p) => acc + p, 0);
                const disconnectedPctSum = guards.reduce((sum, g) => (!g.connected ? sum + g.pct : sum), 0);
                const scale = connectedPctSum > 100 ? 100 / connectedPctSum : 1;
                ...
                // The expected sum is the sum of per-guard max(proportionalShare, failsafeLimit)
                let expectedSum = 0;
                for (let j = 0; j < connectedIndices.length; j++) {
                    const i = connectedIndices[j];
                    const proportionalShare = (effectivePcts[j] * totalLimit) / 100;
                    expectedSum += Math.max(proportionalShare, guards[i].failsafeLimit);
                }

                const tolerance = 1e-6 * Math.abs(expectedSum) + 1e-10;

                expect(connectedSum).to.be.at.most(
                    expectedSum + tolerance,
                    `Connected sum ${connectedSum} exceeds expected sum ${expectedSum}`,
                );
            }),
            { numRuns: 100 },
        );
...
describe('Feature: add-lpp-use-case, Property 4: Effective Limit Calculation', () => {
     ...
     *
     * For any total production limit, any effective percentage (0–100), and any
     * failsafe limit, the calculated effective limit for a single connected guard
     * SHALL equal max(effectivePercentage * totalLimit / 100, failsafeLimit).
     */
    it('effectiveLimit equals max(effectivePercentage * totalLimit / 100, failsafeLimit)', function () {
        this.timeout(30000);
        ...
        fc.assert(
            fc.property(
                ...
                (totalLimit, effectivePercentage, failsafeLimit) => {
                    ...
                    const expectedLimit = Math.max((effectivePercentage * totalLimit) / 100, failsafeLimit);
                    ...
                    expect(diff).to.be.at.most(
                        ...
                        `effectiveLimit=${result[0].effectiveLimit} should equal ` +
                            `max(${effectivePercentage} * ${totalLimit} / 100, ${failsafeLimit}) = ${expectedLimit}`,
                    );
                },
            ),
            { numRuns: 100 },
