...
class BaseEnergyGuard {
     ...
     * @param {number} [_durationNs] - Remaining limit duration in nanoseconds (used by subclasses)
     */
    async applyEffectiveLimit(effectiveLimit, _durationNs = 0) {
     ...
     * Enforces a minimum of 1W so that a failsafe situation is always
     * distinguishable from "no limit" in ioBroker states.
     * Returns true if the value changed, false if unchanged.
     *
     * @param {number} value - The failsafe limit in watts (clamped to minimum 1W)
     * @returns {Promise<boolean>} true if value changed
     */
    async setFailsafeLimit(value) {
        const clamped = Math.max(value, 1);
        if (this.#failsafeLimit === clamped) {
            return false;
        }
        this.#failsafeLimit = clamped;
        await this.#adapter.setStateAsync(`${this.#basePath}.failsafeLimit`, clamped, true);
        return true;
    }
...
class EebusEnergyGuard extends BaseEnergyGuard {
     ...
     *
     * @param {number} effectiveLimit - The final limit value in watts
     * @param {number} [durationNs] - Remaining limit duration in nanoseconds (0 = indefinite)
     */
    async applyEffectiveLimit(effectiveLimit, durationNs = 0) {
        await super.applyEffectiveLimit(effectiveLimit, durationNs);
        ...
        if (this.#egLpcClient && this.#connected && this.#remoteSki) {
            ...
            if (durationNs > 0) {
                limitMsg.duration_nanoseconds = durationNs;
            }
            await callUnary(this.#egLpcClient, this.#writeLimitRpc, {
                instance_id: this.#instanceId,
                remote_ski: this.#remoteSki,
