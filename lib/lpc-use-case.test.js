...
describe('LpcUseCase distributeConsumptionLimit', () => {
    ...
    it('effective limit is at least the failsafe limit', () => {
        ...
        // 10% of 10000 = 1000, but failsafe is 5000 → effective = 5000
        expect(results[0].effectiveLimit).to.equal(5000);
    });

    it('redistributes disconnected shares to connected guards', () => {
        const guards = [
            { pct: 50, connected: true, failsafeLimit: 0 },
        ...
        // Guard 0 is connected, guard 1 is not.
        // Guard 0 gets its 50% + redistributed 50% from disconnected guard = 100% → 10000W
        expect(results[0].effectiveLimit).to.be.closeTo(10000, 1);
        expect(results[1].skip).to.equal(true);
    });
...
// ─── handleGuardStateChange Tests ────────────────────────────────────────────

describe('LpcUseCase handleGuardStateChange', () => {
    it('routes percentage state change to the correct guard', async () => {
        const adapter = createMockAdapter();
        const lpc = new LpcUseCase(adapter, { energyGuards: [] }, {}, key => key);

        // handleGuardStateChange with unrecognized guard name should just return silently
        await lpc.handleGuardStateChange('LPC.EnergyGuards.Guard_Unknown.percentage', { val: 50 });
        // No error logged for non-matching guard
        expect(adapter.log.error.called).to.equal(false);
    });

    it('ignores state change with invalid path format', async () => {
        const adapter = createMockAdapter();
        const lpc = new LpcUseCase(adapter, { energyGuards: [] }, {}, key => key);

        // Too short path should be ignored
        await lpc.handleGuardStateChange('invalid', { val: 50 });
        expect(adapter.log.error.called).to.equal(false);
    });

    it('ignores state change for folder not starting with Guard_', async () => {
        const adapter = createMockAdapter();
        const lpc = new LpcUseCase(adapter, { energyGuards: [] }, {}, key => key);

        await lpc.handleGuardStateChange('LPC.EnergyGuards.NotAGuard.percentage', { val: 50 });
        expect(adapter.log.error.called).to.equal(false);
    });
});
