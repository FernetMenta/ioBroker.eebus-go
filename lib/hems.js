...
const { EgLpc } = require('./eg-lpc');
const { EgLpp } = require('./eg-lpp');
 ...
 *  - Instantiation and lifecycle of LpcUseCase/LppUseCase (CS-side) and EgLpc/EgLpp (EG-side)
 *  - Wiring callbacks between CS use cases and EG classes
 *  - Routing state changes to the correct EG instance
 *
 * CS-specific logic (FSM, CS clients, event handling) is delegated to LpcUseCase / LppUseCase.
 * EG-specific logic (guard init, distribution, failsafe reporting) is delegated to EgLpc / EgLpp.
 */
class Hems {
    #adapter;
    #config;
    ...
    async handleEnergyGuardStateChange(id, state) {
        ...
        if (localId.startsWith('LPC.')) {
            if (this.#egLpc) {
                await this.#egLpc.handleGuardStateChange(localId, state);
            } else {
                log.debug(`LPC state change received but LPC use case not active: ${localId}`);
            }
        } else if (localId.startsWith('LPP.')) {
            if (this.#egLpp) {
                await this.#egLpp.handleGuardStateChange(localId, state);
            } else {
    ...
    async #onRestart() {
        ...
        // (preserves retryTimer which we manage below)
        const savedTimer = this.#retryTimer;
        this.#retryTimer = null; // prevent stop() from clearing our timer
        this.stop();
        this.#retryTimer = savedTimer;
    ...
    async #connect() {
        ...
        // Instantiate use case classes with callbacks to EG layer
        if (lpcEnabled) {
            this.#lpcUseCase = new LpcUseCase(this.#adapter, config, this.#controlClient, this.#translate, {
                onLimitUpdate: async (isActive, limitValue, durationNs) => {
                    if (this.#egLpc) {
                        await this.#egLpc.distributeLimit(isActive, limitValue, durationNs);
                    }
                },
                onControlBoxConnected: async connected => {
                    if (this.#egLpc) {
                        await this.#egLpc.setControlBoxConnected(connected);
                    }
                },
                getGuardFailsafeLimits: () => {
                    return this.#egLpc ? this.#egLpc.getGuardFailsafeLimits() : [];
                },
                onGrpcError: () => this.#scheduleReconnect(),
            });
        } else {
            this.#lpcUseCase = null;
        }
        ...
        if (lppEnabled) {
            this.#lppUseCase = new LppUseCase(this.#adapter, config, this.#controlClient, this.#translate, {
                onLimitUpdate: async (isActive, limitValue, durationNs) => {
                    if (this.#egLpp) {
                        await this.#egLpp.distributeLimit(isActive, limitValue, durationNs);
                    }
                },
                onControlBoxConnected: async connected => {
                    if (this.#egLpp) {
                        await this.#egLpp.setControlBoxConnected(connected);
                    }
                },
                getGuardFailsafeLimits: () => {
                    return this.#egLpp ? this.#egLpp.getGuardFailsafeLimits() : [];
                },
                onGrpcError: () => this.#scheduleReconnect(),
            });
        } else {
        ...
        if (this.#lpcUseCase) {
            this.#egLpc = new EgLpc(
                this.#adapter,
                config,
                this.#controlClient,
                this.#lpcUseCase.csClient,
                this.#lpcUseCase.csInstanceId,
                this.#translate,
                { onGrpcError: () => this.#scheduleReconnect() },
            );
            await this.#egLpc.start(egEntityMap);
        }
        if (this.#lppUseCase) {
            this.#egLpp = new EgLpp(
                this.#adapter,
                config,
                this.#controlClient,
                this.#lppUseCase.csClient,
                this.#lppUseCase.csInstanceId,
                this.#translate,
                { onGrpcError: () => this.#scheduleReconnect() },
            );
            await this.#egLpp.start(egEntityMap);
        }
    ...
    #subscribeDiscoveryEvents() {
        ...
        this.#discoveryStream.on('error', err => {
            if (this.#stopped) {
                log.debug(`Discovery stream closed: ${err.message}`);
                return;
            }
        ...
        this.#discoveryStream.on('end', () => {
            if (this.#stopped) {
                log.debug('Discovery stream ended (shutdown)');
                return;
            }
    ...
    #scheduleReconnect() {
        if (this.#stopped || this.#retryTimer) {
            return; // shutting down or already scheduled
        }
        const log = this.#adapter.log;
