...
class Hems {
    ...
    async handleEnergyGuardStateChange(id, state) {
        ...
        if (stateName === 'percentage') {
            ...
            // Manual limit write: only applies to EebusEnergyGuard, only when control box is offline
            if (guard instanceof EebusEnergyGuard) {
                if (this.#controlBoxConnected) {
                    log.warn(`Rejecting manual limit for guard "${guardName}" — control box is connected`);
