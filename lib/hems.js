...
class Hems {
    ...
    #onEnterState(lifecycle) {
        ...
        if (lifecycle.to === STATE.INIT) {
            ...
            this.#adapter.setState('info.limit', 0, true);
            this.#adapter.setState('info.limitDuration', 0, true);
        } else if (lifecycle.to === STATE.UNLIMITED_AUTONOMOUS) {
            ...
            this.#setControlBoxConnected(false);
            // Write failsafe limit to info.limit
    ...
    async #setControlBoxConnected(connected) {
        ...
        const log = this.#adapter.log;
        log.info(`Control box connection state: ${connected}`);
            ...
            for (const guard of this.#energyGuards) {
                if (guard instanceof EebusEnergyGuard) {
                    try {
                        ...
                        log.warn(`Failed to reset manual limit for guard "${guard.name}": ${err.message}`);
                    }
                }
            }
        }
    ...
    /**
     * Create ioBroker info objects with translated names.
     * Called once during the first connect to ensure objects exist.
     */
    async #createInfoObjects() {
        const t = this.#translate;

        await this.#adapter.extendObjectAsync('info.state', {
            type: 'state',
            common: {
                name: t('HEMS FSM State'),
                type: 'string',
                role: 'text',
                def: 'init',
                read: true,
        ...
        await this.#adapter.extendObjectAsync('info.limit', {
            type: 'state',
            common: {
                name: t('Control Box Limit'),
                type: 'number',
                role: 'value.power',
                unit: 'W',
                def: 0,
        ...
        await this.#adapter.extendObjectAsync('info.limitDuration', {
            type: 'state',
            common: {
                name: t('Control Box Limit Duration'),
                type: 'number',
                role: 'value',
                unit: 'min',
                def: 0,
        ...
        await this.#adapter.extendObjectAsync('info.limitMinutesToday', {
            type: 'state',
            common: {
                name: t('Limit Active Minutes Today'),
                type: 'number',
                role: 'value',
                unit: 'min',
                def: 0,
    ...
    async #initEnergyGuards() {
        ...
        // Create top-level EnergyGuards folder
        await this.#adapter.extendObjectAsync('EnergyGuards', {
            type: 'folder',
            common: { name: 'Energy Guards' },
            native: {},
        });
        ...
        for (const entry of guardConfigs) {
            if (entry.type === 'eebus') {
                ...
                try {
                    ...
                    // Create the entity first, then add the use case to it
     ...
     * Compares existing Guard_* channels under EnergyGuards with the current config
    ...
    async #cleanupRemovedGuards(guardConfigs) {
        ...
        const expectedNames = new Set(guardConfigs.map(entry => `EnergyGuards.Guard_${entry.name}`));

        try {
            // Get all objects under the EnergyGuards folder
            ...
            const channelIds = Object.keys(objects).filter(id => {
                ...
                return localId.startsWith('EnergyGuards.Guard_') && objects[id].type === 'channel';
            });

            for (const fullId of channelIds) {
                const localId = fullId.replace(`${adapterNamespace}.`, '');
                if (!expectedNames.has(localId)) {
                    log.info(`Removing objects for deleted energy guard: ${localId}`);
            ...
            log.warn(`Failed to clean up removed energy guard objects: ${err.message}`);
        }
    }
    ...
    #startLimitTracking() {
        ...
        this.#limitMinutesAccumTimer = setInterval(async () => {
            this.#limitMinutesToday += 1;
            await this.#adapter.setStateAsync('info.limitMinutesToday', this.#limitMinutesToday, true);
        }, 60_000);
    ...
    async #stopLimitTracking() {
        ...
        if (this.#limitMinutesAccumTimer) {
            clearInterval(this.#limitMinutesAccumTimer);
            this.#limitMinutesAccumTimer = null;
        }
    ...
    #scheduleMidnightReset() {
        ...
        this.#midnightResetTimer = setTimeout(async () => {
            this.#midnightResetTimer = null;
            this.#limitMinutesToday = 0;
            await this.#adapter.setStateAsync('info.limitMinutesToday', 0, true);
            this.#adapter.log.info('Midnight reset: info.limitMinutesToday reset to 0');
     ...
     * falls back to applying each guard's own failsafe limit. Otherwise,
    ...
    async #distributeLimit(isActive) {
        ...
        // Read the current controlbox limit value from the info state
        let controlboxLimit = 0;
        try {
            const limitState = await this.#adapter.getStateAsync('info.limit');
        ...
        if (controlboxLimit <= 0) {
            this.#adapter.log.warn('Controlbox limit is active but value is 0 — skipping distribution');
            return;
        }
                    ...
                    try {
                        ...
                        if (manualState && manualState.val) {
                            this.#adapter.log.info(
                                `Controlbox limit activated — resetting manual limit for guard "${guard.name}"`,
                            );
                            await guard.onManualLimitWrite(0);
                            await this.#adapter.setStateAsync(`${guard.basePath}.manualLimit`, 0, true);
                        }
                        ...
                        this.#adapter.log.warn(
                            `Failed to reset manual limit for guard "${guard.name}": ${err.message}`,
                        );
                    }
                }
            }
        ...
        for (let i = 0; i < this.#energyGuards.length; i++) {
            ...
            try {
                await this.#energyGuards[i].applyLimit(controlboxLimit, effectivePct);
            } catch (err) {
                this.#adapter.log.warn(
    ...
    #subscribeEgLpcEvents() {
        ...
        for (const [ski, { endpoint, entityAddr }] of this.#egLpcEndpoints) {
            ...
            stream.on('data', async evt => {
                ...
                log.debug(
                    `EG-LPC event for entity [${entityAddr}]: remote_ski="${remoteSki || ''}", event=${eventName}, full_evt=${JSON.stringify(evt)}`,
                );
                ...
                if (eventName === 'UseCaseSupportUpdate') {
                    ...
                    try {
                        ...
                        log.warn(`Failed to handle DataUpdateLimit for guard "${guard.name}": ${err.message}`);
                    }
                } else {
        ...
        this.#lpcStream = subscribeCsLpcEvents(this.#controlClient, cemAddr, async evt => {
            ...
            if (eventName === 'UseCaseSupportUpdate') {
                ...
                if (evt.remote_ski === this.#config.controlboxSki) {
                    this.#setControlBoxConnected(true);
                }
            } else {
    ...
    /**
     * Approve or deny pending consumption limits based on energy guard failsafe power sum.
     *
     * Approval rules:
     * - Approve if failsafeSum <= limit value (the controlbox leaves enough headroom)
     * - Deny if failsafeSum > limit value (guards cannot respect their own failsafe floors)
     * - If approved but limit < contractualConsumptionNominalMax, log a warning and still approve
     *
    ...
    async #approvePendingLimits() {
        ...
        // Sanity check: load_limits must be a plain object (map<uint64, LoadLimit>).
        // If it's not an object or is an array/string, skip processing.
        ...
        // map<uint64, LoadLimit> keys are decoded by grpc-js as raw 8-byte little-endian
        // binary strings rather than numeric strings. Convert them to JS numbers.
        /**
        ...
        function decodeMsgCounter(key) {
            // If it looks like a plain numeric string, use it directly
            if (/^\d+$/.test(key)) {
                return Number(key);
            }
            // Decode as little-endian uint64 (8 bytes)
            let result = 0;
        ...
        // Calculate sum of all energy guards' failsafe limits
        const failsafeSum = this.#energyGuards.reduce((sum, guard) => sum + guard.failsafeLimit, 0);

        for (const [msgCounter, loadLimit] of entries) {
            // Each entry must be an object with a numeric value field — skip if malformed
            ...
            // Deny if the limit is below the combined guard failsafe floors —
            // applying it would force guards below their own minimum safe power.
            if (failsafeSum > value) {
                const reason = `Limit ${value}W is below sum of guard failsafe limits (${failsafeSum}W)`;
                log.warn(`Denying pending limit msgCounter=${msgCounter} — ${reason}`);
                ...
                if (this.#can('heartbeatLimitNotApplicable')) {
            ...
            if (value < contractualMax) {
                log.warn(
                    `Approving pending limit msgCounter=${msgCounterNum}, value=${value}W — ` +
    ...
    async #handleDataUpdateLimit() {
        ...
        // Update info.limit: show active limit value, 0 if deactivated
        const limitValue = limit.is_active ? limit.value || 0 : 0;
        this.#adapter.setState('info.limit', limitValue, true);

        // Update info.limitDuration: show duration in minutes, 0 if deactivated or indefinite
        ...
        this.#adapter.setState('info.limitDuration', durationMin, true);
    ...
    async #handleDataUpdateFailsafeLimit() {
        ...
        // Treat failsafe limit as active limit — update info.limit if in failsafe state
        if (this.state === STATE.FAILSAFE) {
            this.#adapter.setState('info.limit', res.limit || 0, true);
        }
    }

    /**
     * Read the failsafe limit and write it to info.limit.
    ...
    async #updateFailsafeLimitState() {
        try {
            ...
            this.#adapter.log.warn(`Failed to read failsafe limit for info.limit: ${err.message}`);
        }
    }
    ...
    async handleEnergyGuardStateChange(id, state) {
        ...
        if (stateName === 'percentage') {
            ...
            if (guard instanceof ManualEnergyGuard) {
                ...
                log.info(`EEBUS energy guard "${guardName}" failsafeLimit changed to ${state.val}`);
                try {
                    await guard.onFailsafeLimitWrite(state.val);
                    // Failsafe limit change affects distribution if limit is active
                    if (this.#lastLimitActive) {
                    ...
                    log.warn(`Failed to write failsafe limit for guard "${guardName}": ${err.message}`);
                }
            }
        } else if (stateName === 'manualLimit') {
            // Manual limit write: only applies to EebusEnergyGuard
            // Allowed when:
            //  - control box is not connected, OR
            //  - control box is connected but has no active limit
            if (guard instanceof EebusEnergyGuard) {
                if (this.#controlBoxConnected && this.#lastLimitActive) {
                    log.warn(`Rejecting manual limit for guard "${guardName}" — control box has an active limit`);
                    // Reset the state back to 0
                    await this.#adapter.setStateAsync(`${guard.basePath}.manualLimit`, 0, true);
                    return;
                }
                log.info(`EEBUS energy guard "${guardName}" manualLimit changed to ${state.val}`);
                try {
                    await guard.onManualLimitWrite(state.val);
                    // Confirm acceptance by setting ack=true
                    await this.#adapter.setStateAsync(`${guard.basePath}.manualLimit`, state.val, true);
                } catch (err) {
                    log.warn(`Failed to write manual limit for guard "${guardName}": ${err.message}`);
                }
            }
        }
    }
